Report_14036nodb<-function(datasets)
{
    ...
    onstudy_tx = datasets$onstudy.CSV %>% rename_all(tolower)%>%dplyr::select(subject,initialtxdate_raw,doseassign)
    ...
    #################### Extracting the total dose for Eribulin and Everolin 
    study_dose = datasets$studydrug.CSV %>%rename_all(tolower)%>%filter(instancerepeatnumber==0)%>%dplyr::select(subject,agent,agent_std,totaldose)

    #sort in increasing order by Subject and agent
    study_dose_ord = study_dose[order(study_dose$subject,study_dose$agent_std),]
    
    #cast to wide format with Agent as column
    study_cast = dcast(study_dose_ord,subject~agent,value.var="totaldose")
    ...
    mytable = Reduce(function(x, y) merge(x, y, all=TRUE), list(onstudy_tx,number_cycle_nodup,offs_date_reason ,offtx_date_reason,study_cast,new_fu))
    
    #Find unique subjects in mytable
    mytable =mytable[!duplicated(mytable$subject),]
    
    #mytable["OFFTXDATEYN"]=ifelse(is.na(mytable$OFFTXDATE_RAW),"No","Yes")
    ...
    #reorder columns
    mytable = mytable[c("Subject","DOSEASSIGN","INITIALTXDATE_RAW","NUMBER OF TREATMENT","ERIBULIN (IV)","EVEROLIMUS (ORAL)","DAYS_NUMBER","PROGRESSIONYN","PROGRESSIONDATE_FORMAT","BESTRESPFU","LASTCONTACTDATE_FORMAT","OFFSTUDYDATE_RAW","OFFSTUDYREASON","OFFTXDATE_RAW","OFFTXREASON","VITALSTATUS","DEATHDATE_FORMAT")]
    
    #rename columns
    mytable = rename(mytable, "Subject"="Subject","Dose Assignment"="DOSEASSIGN","Date of Treatment"="INITIALTXDATE_RAW" ,"Number of Treatment"="NUMBER OF TREATMENT","Total dose of Eribulin (mg)"="ERIBULIN (IV)","Total dose of Everolimus (mg)"="EVEROLIMUS (ORAL)","Days from treatment to progression \n or if not progressed to the last contact date"="DAYS_NUMBER",
