Report_demo_13384_nodb <- function(datasets)
{
    ...
    ######### cleaning strata column ########################
    #extract desired columns from protocolproced form
    ...
    ######### merge clean files #############################
    merged = Reduce(function(x, y) merge(x, y), list(tcelladmin_unique,demo_select,strata_clean))
    ...
    split_by_strata = split(merged_nodupl,merged_nodupl$RECODE)
    ...
    colnames(age_table_df)<- cnames
    
    #### Table for Gender
    gender_table = lapply(split_by_strata,function(x) as.data.frame(table(factor(x$gender,levels=c("Male","Female")))))
    ...
    #Rename columns' names
    colnames(eth_table_df)<-cnames
    
    #### Table for Race
    #count number of subjects per race
    extract_race = lapply(split_by_strata, function(x) as.data.frame(x[,c("AI","A/PI","B","W","O","UNK")]))
    ...
    colnames(combined_label_df)<-NULL
