Report_death_13401_nodb <- function(datasets)
{
    ...
    #last three rows is the same subject but there is a mispelling in the "Cause of Death" that cause it to repeat 3 times.
    merged_clean2 = merged_clean2[!duplicated(merged_clean2$Subject),]
    
    rownames(merged_clean2)<-c()
    
    #death13401_report = merged_clean2
    merged_clean2
}
